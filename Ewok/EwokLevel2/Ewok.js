...
treeLoader.load('GLTF/scene.gltf', (gltf) => {
	xwing = gltf.scene;
	xwing.scale.set(4.8, 4.8, 4.8);
	xwing.rotation.y = Math.PI * 2 + 1;
	xwing.position.set(-50, 58, -100);
	scene.add(xwing);
	
});
treeLoader.load('Ewokspeedermodel2.glb', (gltf) => {
...
function updateAnimation() {
	...
	const offset = new THREE.Vector3(0, 25, -40); // Adjust this offset as needed
...
function createTerrain() {
    ...
    function animate() {
		...
		if (enterPressed === false) {
			...
			const angle = atstModel.rotation.y;
			if(upPressed) {
				atstModel.position.y += 0.5;
				atstModel.position.x += 1.4 * Math.sin(angle);
        		atstModel.position.z += 1.4 * Math.cos(angle);
